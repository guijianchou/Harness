...
Full release notes for Harness (formerly OpenClaw Manager). See [README.md](README.md) / [readme_zh.md](readme_zh.md) for project overview.
...
- Added a General > Auto refresh switch. When disabled, automatic session recovery, navigation retries, heartbeat-triggered reloads, and timeout-triggered WebView recreation are suppressed; manual Reload/Retry, startup, and environment changes remain available.
- Added regression coverage for the Auto refresh default, persistence, suppression, and cancellation paths.
- Kept System tray disabled by default and shortened its General-page description.

### v5.3.2 (2026-08-30)

- Updated app, assembly, file, package manifest, application manifest, README, Chinese README, and changelog metadata to `5.3.2`.
- Merged the separate Minimize to tray and Close to tray options into one System tray setting. Minimizing now always stays on the taskbar; with System tray enabled, closing hides Harness to the system tray, and with it disabled, closing exits the app.
- Added regression coverage for legacy tray-setting migration and the merged close policy.

### v5.3.1 (2026-08-22)

- Updated app, assembly, file, package manifest, application manifest, README, Chinese README, and changelog metadata to `5.3.1`.
- The top status bar's latency badge now reads `N/A` instead of `ERR` when the Control UI latency probe fails. The failure colour is unchanged, so a failed probe still reads as a failure without the harsher wording.

### v5.3.0 (2026-08-22)

- Updated app, assembly, file, package manifest, application manifest, README, Chinese README, and changelog metadata to `5.3.0`.
- Renamed the application from `OpenClaw Manager` to `Harness`, marking the shift from an OpenClaw-only client to a general client for hosted agent Control UIs (OpenClaw and DeepSeek Harness). This release is the rename only; no backend abstraction was added and OpenClaw remains the only working backend.
- Renamed every project and the solution: `src/OpenClaw/OpenClaw.csproj` → `src/Harness/Harness.csproj`, `src/OpenClaw.Core/OpenClaw.Core.csproj` → `src/Harness.Core/Harness.Core.csproj`, `tests/OpenClaw.Core.Tests` → `tests/Harness.Core.Tests`, and `OpenClaw.sln` → `Harness.sln`. The shell now builds `Harness.exe`.
- Moved all namespaces from `OpenClaw.*` to `Harness.*` across the shell, Core, and test assemblies, and renamed the embedded bridge/command/inspection JS resources from `OpenClaw.Services.*` to `Harness.Services.*`.
- Renamed the host↔bridge message kinds `openclaw-session-ready`, `openclaw-event-gap`, and `openclaw-control-ui-status` to their `harness-` equivalents. These are an internal contract between the injected bridge script and the host, not part of the OpenClaw page API.
- Renamed the single-instance lock and named pipe from `OpenClaw.Manager.SingleInstance` to `Harness.SingleInstance`.
- **Breaking:** moved local data from `%LOCALAPPDATA%\OpenClaw\` to `%LOCALAPPDATA%\Harness\` with no automatic migration, so 5.3.0 starts with a clean profile: gateway environments must be re-added in Settings and hosted Control UI logins must be redone. The old directory is left untouched on disk. Log and diagnostic file names moved with it (`harness-<date>.log`, `harness-diagnostics-<timestamp>.zip`), and the retention/log-viewer/diagnostic-bundle globs were updated to match.
- Replaced the application artwork with the new Harness whale logo. `Assets/harness-logo.png` is the source; the window/tray icon, tile, splash, store, lock-screen, and in-app brand images were regenerated from it. `WindowIcon.ico` is now multi-resolution (16–256px) instead of a single 32px frame, and the outer white background was keyed out so the mark sits correctly on light and dark title bars and tray backgrounds.
- Fixed two pre-existing publish defects that made `dotnet publish` output unusable. The generated `.pri` was not copied, so a published build threw `XamlParseException` from the first `InitializeComponent` because the compiled XAML and `.resw` tables were absent; and `Assets/**` was copied to the build output but not the publish output, so a published build logged "Failed to load tray icon: 2" and ran with no window or tray icon. `Harness.csproj` now adds the PRI to the publish manifest and sets `CopyToPublishDirectory` on the asset content.
- Fixed the hardcoded fallback tray label in Core, which still read "Open OpenClaw" when localized resources were unavailable.
- Rebranded user-visible surfaces: window title, top-bar brand, About dialog, tray tooltip and menu, package/application manifests, and the English/Chinese tray, hotkey, and multi-instance strings.
- Left unchanged on purpose: every string and identifier that describes the OpenClaw *gateway* rather than this client — gateway wording and `openclaw devices ...` hints in the localized resources, the documented `__openclaw__/a2ui/` probe path, `openclaw-app` DOM probing and the `readOpenClaw*` bridge helpers, and the `OPENCLAW_NODE` / `OPENCLAW_ALLOW_NODE_SKIP` build-script variables.
...
- 将 app、assembly、file、package manifest、application manifest、README、中文 README 和 changelog 元数据同步到 `5.3.3`。
- 在 General 中增加 Auto refresh 开关。关闭后会抑制自动会话恢复、导航重试、心跳触发的刷新和超时触发的 WebView 重建；手动 Reload/Retry、启动初始化和环境切换仍然可用。
- 增加了 Auto refresh 默认值、持久化、抑制和取消路径的回归测试。
- System tray 继续默认关闭，并缩短 General 页面中的介绍文案。

### v5.3.2 (2026-08-30)

- 将 app、assembly、file、package manifest、application manifest、README、中文 README 和 changelog 元数据同步到 `5.3.2`。
- 将原来的 Minimize to tray 和 Close to tray 合并为一个 System tray 开关。最小化现在始终留在任务栏；开启 System tray 时点击关闭会隐藏到系统托盘，关闭时点击关闭会退出应用。
- 增加了 legacy 托盘设置迁移和合并后关闭策略的回归测试。

### v5.3.1 (2026-08-22)

- 将 app、assembly、file、package manifest、application manifest、README、中文 README 和 changelog 元数据同步到 `5.3.1`。
- 顶部状态栏的延迟徽标在 Control UI 延迟探测失败时显示 `N/A`，不再显示 `ERR`。失败配色保持不变，探测失败仍然能被识别为失败，只是措辞不再那么刺眼。

### v5.3.0 (2026-08-22)

- 将 app、assembly、file、package manifest、application manifest、README、中文 README 和 changelog 元数据同步到 `5.3.0`。
- 应用从 `OpenClaw Manager` 改名为 `Harness`，标记从"仅 OpenClaw 客户端"到"托管 agent Control UI 通用客户端"（OpenClaw 与 DeepSeek Harness）的定位转变。本版本仅做改名，没有引入后端抽象，目前可用的后端仍然只有 OpenClaw。
- 全部项目和解决方案改名：`src/OpenClaw/OpenClaw.csproj` → `src/Harness/Harness.csproj`，`src/OpenClaw.Core/OpenClaw.Core.csproj` → `src/Harness.Core/Harness.Core.csproj`，`tests/OpenClaw.Core.Tests` → `tests/Harness.Core.Tests`，`OpenClaw.sln` → `Harness.sln`。外壳现在产出 `Harness.exe`。
- 外壳、Core 和测试程序集的全部命名空间从 `OpenClaw.*` 迁移到 `Harness.*`，内嵌的 bridge/command/inspection JS 资源名从 `OpenClaw.Services.*` 改为 `Harness.Services.*`。
- 宿主↔bridge 的消息类型 `openclaw-session-ready`、`openclaw-event-gap`、`openclaw-control-ui-status` 改为对应的 `harness-` 前缀。它们是注入脚本与宿主之间的内部契约，不属于 OpenClaw 页面 API。
- 单实例锁和命名管道从 `OpenClaw.Manager.SingleInstance` 改为 `Harness.SingleInstance`。
- **破坏性变更：** 本地数据目录从 `%LOCALAPPDATA%\OpenClaw\` 迁到 `%LOCALAPPDATA%\Harness\`，且不自动搬运，因此 5.3.0 会以全新配置启动：需要在设置中重新添加网关环境，并重新登录托管 Control UI。旧目录在磁盘上原样保留。日志和诊断文件名一并调整（`harness-<日期>.log`、`harness-diagnostics-<时间戳>.zip`），日志保留、日志查看器和诊断包的匹配模式同步更新。
- 更换应用图标为新的 Harness 鲸鱼 logo。源文件为 `Assets/harness-logo.png`，窗口/托盘图标、磁贴、启动图、商店图、锁屏图和应用内品牌图均由它重新生成。`WindowIcon.ico` 从单一 32px 帧改为多分辨率（16–256px），并抠掉了外层白底，使图标在浅色/深色标题栏和托盘背景下都能正确显示。
- 修复两个此前就存在、导致 `dotnet publish` 产物不可用的缺陷：生成的 `.pri` 没有被复制，发布产物在首个 `InitializeComponent` 抛 `XamlParseException`（编译后的 XAML 和 `.resw` 表缺失）；`Assets/**` 只进了 build 输出而没进 publish 输出，发布产物记录 "Failed to load tray icon: 2" 且没有窗口和托盘图标。`Harness.csproj` 现在显式把 PRI 加入 publish 清单，并给资源 Content 设置 `CopyToPublishDirectory`。
- 修复 Core 中硬编码的托盘兜底文案：本地化资源不可用时仍显示 "Open OpenClaw"。
- 用户可见界面同步改名：窗口标题、顶栏品牌名、关于对话框、托盘 tooltip 与菜单、package/application manifest，以及中英文的托盘、热键和多实例文案。
- 刻意保持不变：所有描述 OpenClaw *网关* 而非本客户端的文案与标识符 —— 本地化资源中的 Gateway 文案和 `openclaw devices ...` 提示、文档中的 `__openclaw__/a2ui/` 探测路径、`openclaw-app` DOM 探测与 `readOpenClaw*` bridge 辅助函数，以及 `OPENCLAW_NODE` / `OPENCLAW_ALLOW_NODE_SKIP` 构建脚本变量。
