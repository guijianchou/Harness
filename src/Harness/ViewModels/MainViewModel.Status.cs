...
public partial class MainViewModel
{
    ...
    private void OnNavigationStartTimedOut(string message)
    {
        DispatchUiUpdate(() =>
        {
            ...
            {
                _runtime.Logger.Info("navigation.start.timeout.auto_refresh_disabled", new { message });
    ...
    private void OnNavigationCompletionTimedOut(string message)
    {
        DispatchUiUpdate(() =>
        {
            if (!AutoRefreshEnabled)
            {
                _runtime.Logger.Info("navigation.completion.timeout.auto_refresh_disabled", new { message });
                ErrorMessage = message;
                IsErrorVisible = true;
                ShowRetryButton = true;
                UpdateStatusPresentation();
