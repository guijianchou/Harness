...
- Run `dotnet format Harness.sln --verify-no-changes --no-restore` with `Platform=x64` before handoff.
...
- Shared visual constants live in focused dictionaries under `src/Harness/Styles/` and are merged from `App.xaml`.
...
- The Core physical source tree (`src/Harness.Core`) owns pure settings, diagnostics formatting, parser, policy, telemetry, and recovery logic.
...
- Core-compatible files physically live under `src/Harness.Core`; do not add linked Core source files unless a migration plan explicitly scopes a short-lived transition.
- `Harness.Core` must remain a platform-independent SDK class library. The solution may expose `x86`, `x64`, and `ARM64` for the WinUI app, but Core mappings must target `Debug|AnyCPU` or `Release|AnyCPU` because VS2026 validates project configurations before build.
...
- Version bumps update `Harness.csproj`, package manifest, application manifest, README, Chinese README, and changelog.
