...
public sealed class ShellSessionCoordinatorTests
{
    ...
    [TestMethod]
    public async Task DisabledAutoRefreshSuppressesRecoveryRequests()
    {
        var webView = new FakeShellWebView();
        var bridge = new FakeShellBridge();
        using var coordinator = new ShellSessionCoordinator();
        await coordinator.AttachAsync(webView, bridge, autoRefresh: false);

        await coordinator.RequestReconnectAsync("disabled_reconnect");
        await coordinator.RequestSoftResyncAsync("disabled_soft_resync");
        await coordinator.RequestHardRefreshAsync("disabled_hard_refresh");
        bridge.RaiseEventGap(new EventGapEventArgs(1, 3, "state-1", "state-3", "now"));
        webView.RaiseHeartbeatFailure("gateway is down");

        Assert.IsFalse(coordinator.AutoRefreshEnabled);
        Assert.AreEqual(0, webView.InspectionCount);
        Assert.AreEqual(0, webView.ReloadCount);
        Assert.AreEqual(0, bridge.SessionRefreshCount);
        Assert.AreEqual(0, bridge.RecentMessagesCount);
        Assert.AreEqual(0, bridge.LightweightSyncCount);
        Assert.AreEqual(0, bridge.ReconnectIntentCount);
    }

    [TestMethod]
    public async Task DisablingAutoRefreshCancelsPendingRecovery()
    {
        var webView = new FakeShellWebView();
        var bridge = new FakeShellBridge();
        using var coordinator = new ShellSessionCoordinator();
        ...
        var hardRefresh = coordinator.RequestHardRefreshAsync("pending_hard_refresh");
        await webView.WaitForInspectionRequestAsync();
        Assert.IsTrue(coordinator.IsRecoveryInProgress);
        ...
        Assert.IsFalse(coordinator.AutoRefreshEnabled);
        Assert.IsFalse(coordinator.IsRecoveryInProgress);
        Assert.AreEqual(0, webView.ReloadCount, "disabling auto-refresh must stop a pending recovery before reload");
        Assert.AreEqual(RecoveryState.Degraded, coordinator.CurrentRecoveryState);
    }
...
#pragma warning disable CS0067 // Fake implementations do not raise every interface event.
    private sealed class FakeShellWebView : IShellSessionWebView
    {
        ...
        public void RaiseHeartbeatFailure(string message) => HeartbeatFailed?.Invoke(message);
    ...
    {
        private int _sessionRefreshCount;
        private int _recentMessagesCount;
        private int _lightweightSyncCount;
        private int _reconnectIntentCount;

        public event Action<SessionReadyEventArgs>? SessionReady;
        public event Action<EventGapEventArgs>? EventGapDetected;

        public int SessionRefreshCount => Volatile.Read(ref _sessionRefreshCount);

        public int RecentMessagesCount => Volatile.Read(ref _recentMessagesCount);

        public int LightweightSyncCount => Volatile.Read(ref _lightweightSyncCount);

        public int ReconnectIntentCount => Volatile.Read(ref _reconnectIntentCount);

        public Task<bool> RequestSessionRefreshAsync(CancellationToken cancellationToken)
        {
            Interlocked.Increment(ref _sessionRefreshCount);
            return Task.FromResult(false);
        }

        public Task<bool> RequestRecentMessagesAsync(CancellationToken cancellationToken)
        {
            Interlocked.Increment(ref _recentMessagesCount);
            return Task.FromResult(false);
        }

        public Task<bool> RequestLightweightSyncAsync(CancellationToken cancellationToken)
        {
            Interlocked.Increment(ref _lightweightSyncCount);
            return Task.FromResult(false);
        }

        public Task<bool> NotifyReconnectIntentAsync(CancellationToken cancellationToken)
        {
            Interlocked.Increment(ref _reconnectIntentCount);
            return Task.FromResult(false);
        }

        public void RaiseEventGap(EventGapEventArgs args) => EventGapDetected?.Invoke(args);
    }
