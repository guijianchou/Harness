...
$testsProjectPath = Join-Path $repoRoot 'tests/Harness.Core.Tests/Harness.Core.Tests.csproj'
if (-not (Test-Path -LiteralPath $testsProjectPath)) {
    throw 'tests/Harness.Core.Tests is missing. The Core regression test project must stay in the repository.'
}

$solution = Get-Content -LiteralPath (Join-Path $repoRoot 'Harness.sln') -Raw
if ($solution -notmatch [regex]::Escape('tests\Harness.Core.Tests\Harness.Core.Tests.csproj')) {
    throw 'Harness.sln must reference tests/Harness.Core.Tests so the regression suite builds with the solution.'
}

$coreProjectId = '{BC4C7184-C8DD-4748-AC82-D26123568BD1}'
$coreProject = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness.Core/Harness.Core.csproj') -Raw
if ($coreProject -match '<Platforms>') {
        throw 'Harness.Core.csproj must stay platform-independent. Do not declare x64/x86/ARM64 platforms on the pure SDK class library; map solution platforms to Debug/Release|Any CPU instead.'
}
...
foreach ($coreSolutionMapping in [regex]::Matches($solution, $coreSolutionMappingPattern)) {
    if ($coreSolutionMapping.Groups[5].Value -ne 'Any CPU') {
        throw "Harness.Core solution platform mappings must target the pure class-library Any CPU configuration; invalid mapping: $($coreSolutionMapping.Value)"
    }
}
...
foreach ($coreSolutionMapping in [regex]::Matches($solution, $allCoreSolutionMappingPattern)) {
    if ($coreSolutionMapping.Groups['configuration'].Value -notin @('Debug', 'Release') -or
        ...
        throw "Harness.Core solution mappings must map Debug/Release x64/x86/ARM64 solution platforms to the matching Debug/Release|Any CPU project configuration: $($coreSolutionMapping.Value)"
    }
}
    ...
    foreach ($platform in @('x64', 'x86', 'ARM64')) {
        foreach ($mapping in @('ActiveCfg', 'Build.0')) {
            ...
            if ($solution -notmatch [regex]::Escape($expectedCoreMapping)) {
                throw "Harness.Core solution platform mapping is missing or invalid: $expectedCoreMapping"
            }
        }
    }
}

$coreFiles = Get-ChildItem -LiteralPath (Join-Path $repoRoot 'src/Harness.Core') -Recurse -File -Include *.cs
...
$approvedSynchronousWaits = @{
    'src/Harness.Core/Services/ConfigurationService.cs' = @(
        'task.Wait(TimeSpan.FromSeconds(2))'
    )
    'src/Harness.Core/Services/LoggingService.cs' = @(
        '_writerTask.Wait(TimeSpan.FromSeconds(2))'
    )
    'src/Harness.Core/Services/SingleInstanceCoordinator.cs' = @(
        'StopAsync().GetAwaiter().GetResult()'
    )
    'src/Harness/App.xaml.cs' = @(
...
$project = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Harness.csproj') -Raw
...
foreach ($linkedCompileItem in $linkedCompileItems) {
    ...
    if ($compilePath -match '(^|[\\/])\.\.[\\/]' -or
        $compilePath -match '(^|[\\/])Harness\.Core([\\/]|$)') {
        throw "Harness.csproj must not link Core source files or compile sources outside the WinUI project: $compilePath"
    }
}
...
if ($project -notmatch [regex]::Escape("<Version>$currentVersion</Version>") -or
    ...
    throw "Harness.csproj version metadata must stay aligned at $currentVersion / $currentFileVersion."
}

$packageManifest = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Package.appxmanifest') -Raw
$appManifest = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/app.manifest') -Raw
...
$englishResources = [xml](Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Strings/en-us/Resources.resw') -Raw -Encoding UTF8)
$chineseResources = [xml](Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Strings/zh-cn/Resources.resw') -Raw -Encoding UTF8)
...
$hostedBridgeScriptBuilder = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/HostedUiBridge.Script.cs') -Raw
$hostedBridgeScriptTemplate = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/HostedUiBridge.Script.js') -Raw
...
$webViewService = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewService.cs') -Raw
...
$webViewNavigation = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewService.Navigation.cs') -Raw
$webViewHostMessages = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewService.HostMessages.cs') -Raw
$webViewNavigationState = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewService.NavigationState.cs') -Raw
$webViewNavigationWatchdogs = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewService.NavigationWatchdogs.cs') -Raw
$webViewNavigationCommands = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewService.NavigationCommands.cs') -Raw
$webViewPageToken = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewService.PageToken.cs') -Raw
$webViewNavigationRecovery = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewService.NavigationRecovery.cs') -Raw
$webViewLifecycle = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewService.Lifecycle.cs') -Raw
$webViewSession = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewService.Session.cs') -Raw
...
$heartbeat = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewService.Heartbeat.cs') -Raw
...
$heartbeatRuntime = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/HeartbeatRuntime.cs') -Raw
...
$latencyService = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness.Core/Services/ControlUiLatencyService.cs') -Raw
...
$app = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/App.xaml.cs') -Raw
...
$webViewServiceFiles = Get-ChildItem -LiteralPath (Join-Path $repoRoot 'src/Harness/Services') -File -Filter 'WebViewService*.cs'
...
$gatewayHeartbeatTransport = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/GatewayHeartbeatTransport.cs') -Raw
...
$hostedSessionHeartbeatPolicy = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/HostedSessionHeartbeatPolicy.cs') -Raw
...
$sessionProbeModels = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness.Core/Services/SessionProbeModels.cs') -Raw
...
$commandFile = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewService.Commands.cs') -Raw
...
$statusInspector = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewStatusInspector.cs') -Raw
$statusInspectorInspection = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewStatusInspector.Inspection.cs') -Raw
$statusInspectorParsing = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewStatusInspector.Parsing.cs') -Raw
$statusInspectorProbe = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewStatusInspector.Probe.cs') -Raw
$statusInspectorScriptExecution = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewStatusInspector.ScriptExecution.cs') -Raw
...
$ownership = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewMessageOwnership.cs') -Raw
...
$webViewServiceMain = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewService.cs') -Raw
...
$hostedUiBridge = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/HostedUiBridge.cs') -Raw
...
$mainViewModelLifecycle = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/ViewModels/MainViewModel.Lifecycle.cs') -Raw
$mainViewModelStatus = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/ViewModels/MainViewModel.Status.cs') -Raw
$mainWindowWebView = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/MainWindow.WebView.cs') -Raw
$navigationStartTimeoutHandler = [regex]::Match($mainViewModelStatus, 'private void OnNavigationStartTimedOut\(string message\)[\s\S]*?\n    \}')
$navigationStartTimeoutDisabledBranch = [regex]::Match($navigationStartTimeoutHandler.Value, 'if \(!AutoRefreshEnabled\)[\s\S]*?return;\s*\}')
$navigationStartTimeoutEnabledBranch = if ($navigationStartTimeoutDisabledBranch.Success) {
    $navigationStartTimeoutHandler.Value.Substring($navigationStartTimeoutDisabledBranch.Index + $navigationStartTimeoutDisabledBranch.Length)
} else {
    ''
}
if ($mainViewModelLifecycle -notmatch 'NavigationStartTimedOut \+= OnNavigationStartTimedOut' -or
    ...
    $mainViewModelLifecycle -notmatch 'NavigationTimeoutRecovered -= OnNavigationTimeoutRecovered' -or
    -not $navigationStartTimeoutHandler.Success -or
    -not $navigationStartTimeoutDisabledBranch.Success -or
    $navigationStartTimeoutEnabledBranch -notmatch 'WebViewRecreationRequested\?\.Invoke\("navigation_start_timeout"\)' -or
    $mainViewModelStatus -notmatch 'navigation\.start\.timeout\.recovery_requested' -or
    $navigationStartTimeoutEnabledBranch -match 'ApplyNavigationError|ErrorOccurred\?\.Invoke|IsErrorVisible = true') {
...
$navigationCompletionTimeoutHandler = [regex]::Match($mainViewModelStatus, 'private void OnNavigationCompletionTimedOut\(string message\)[\s\S]*?\n    \}')
$navigationCompletionTimeoutDisabledBranch = [regex]::Match($navigationCompletionTimeoutHandler.Value, 'if \(!AutoRefreshEnabled\)[\s\S]*?return;\s*\}')
$navigationCompletionTimeoutEnabledBranch = if ($navigationCompletionTimeoutDisabledBranch.Success) {
    $navigationCompletionTimeoutHandler.Value.Substring($navigationCompletionTimeoutDisabledBranch.Index + $navigationCompletionTimeoutDisabledBranch.Length)
} else {
    ''
}
if ($mainViewModelLifecycle -notmatch 'NavigationCompletionTimedOut \+= OnNavigationCompletionTimedOut' -or
    ...
    -not $navigationCompletionTimeoutHandler.Success -or
    -not $navigationCompletionTimeoutDisabledBranch.Success -or
    $navigationCompletionTimeoutEnabledBranch -notmatch 'WebViewRecreationRequested\?\.Invoke\("navigation_completion_timeout"\)' -or
    $mainViewModelStatus -notmatch 'navigation\.completion\.timeout\.recovery_requested' -or
    $navigationCompletionTimeoutEnabledBranch -match 'ApplyNavigationError|ErrorOccurred\?\.Invoke|IsErrorVisible = true') {
...
$mainWindowInitialization = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/MainWindow.Initialization.cs') -Raw
$mainWindowCommands = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/MainWindow.Commands.cs') -Raw
$mainWindowLifecycle = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/MainWindow.Lifecycle.cs') -Raw
$mainWindowShared = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/MainWindow.Shared.cs') -Raw
$mainViewModelShared = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/ViewModels/MainViewModel.Shared.cs') -Raw
$webViewRecreationService = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewRecreationService.cs') -Raw
...
$recreationService = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewRecreationService.cs') -Raw
$circuitBreaker = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness.Core/Services/WebViewCircuitBreaker.cs') -Raw
...
$navigationCancellationScope = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/NavigationCancellationScope.cs') -Raw
...
$hostedBridgeMain = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/HostedUiBridge.cs') -Raw
...
$hostMessagingScript = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/HostedUiBridge.HostMessaging.js') -Raw
...
foreach ($asset in @('WebViewCommands.StopInjection.js', 'WebViewCommands.AbortRun.js')) {
    if (-not (Test-Path -LiteralPath (Join-Path $repoRoot "src/Harness/Services/$asset"))) {
...
$environmentConfig = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness.Core/Models/EnvironmentConfig.cs') -Raw
...
$mainViewModelEnvironment = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/ViewModels/MainViewModel.Environment.cs') -Raw
$mainViewModelConstructor = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/ViewModels/MainViewModel.cs') -Raw
if ($mainViewModelLifecycle -notmatch 'if \(_selectedEnvironment\.IsPlaceholder\)[\s\S]*ApplyPlaceholderEnvironmentState\(\)[\s\S]*return;' -or
    ...
    $mainViewModelEnvironment -notmatch 'ApplyConnectionState\((?:Harness\.Services\.)?ConnectionState\.Offline\)' -or
...
$settingsDialogActions = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Views/SettingsDialog.Actions.cs') -Raw
...
foreach ($resourceFile in @(
    'src/Harness/Strings/en-us/Resources.resw',
    'src/Harness/Strings/zh-cn/Resources.resw'
)) {
...
$coordinatorEvents = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness.Core/Services/ShellSessionCoordinator.Events.cs') -Raw
...
$coordinatorAttach = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness.Core/Services/ShellSessionCoordinator.Attach.cs') -Raw
...
$coordinatorStateEffects = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness.Core/Services/ShellSessionCoordinator.StateEffects.cs') -Raw
...
$coordinatorHost = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness.Core/Services/ShellSessionCoordinator.Host.cs') -Raw
...
$coordinatorRecoveryLifecycle = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness.Core/Services/ShellSessionCoordinator.RecoveryLifecycle.cs') -Raw
...
$settingsViewModel = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/ViewModels/SettingsViewModel.cs') -Raw
if ($settingsViewModel -match 'using Harness\.Views;') {
...
if ($settingsViewModel -notmatch '_didEditAlwaysOnTop' -or
    ...
    $settingsViewModel -notmatch '_didEditAutoRefresh' -or
    ...
    $settingsViewModel -notmatch 'if \(_didEditAutoRefresh\)[\s\S]*settings\.AutoRefresh = AutoRefresh' -or
...
$settingsPersistenceAdapter = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/SettingsPersistenceAdapter.cs') -Raw
...
$configurationService = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness.Core/Services/ConfigurationService.cs') -Raw
...
$settingsDialogShared = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Views/SettingsDialog.Shared.cs') -Raw
if ($settingsDialogShared -match 'record struct SettingsSaveResult') {
    throw 'SettingsSaveResult must live in Harness.Core models, not SettingsDialog.Shared.cs.'
}

$settingsDialogTheme = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Views/SettingsDialog.Theme.cs') -Raw
...
$appSettings = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness.Core/Models/AppSettings.cs') -Raw
$settingsDialogInitialization = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Views/SettingsDialog.Initialization.cs') -Raw
...
$compact = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/MainWindow.CompactMode.cs') -Raw
...
$mainWindowXaml = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/MainWindow.xaml') -Raw
...
$mainBridgeScript = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/HostedUiBridge.Script.js') -Raw
...
$statusInspectionScript = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/HostedUiBridge.StatusInspection.js') -Raw
...
$dependencies = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness.Core/Services/ShellSessionCoordinator.Dependencies.cs') -Raw
...
$recovery = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness.Core/Services/ShellSessionCoordinator.Recovery.cs') -Raw
...
$coordinatorRecoveryInspection = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness.Core/Services/ShellSessionCoordinator.RecoveryInspection.cs') -Raw
...
$adapter = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/ShellSessionCoordinator.Adapters.cs') -Raw
...
$webViewInspection = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/WebViewService.ControlUiInspection.cs') -Raw
...
$uiTaskDispatcher = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/UiTaskDispatcher.cs') -Raw
...
$mainWindow = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/MainWindow.xaml.cs') -Raw
...
$mainViewModel = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/ViewModels/MainViewModel.cs') -Raw
...
$viewModelStatus = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/ViewModels/MainViewModel.Status.cs') -Raw
...
$viewModelHeartbeat = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/ViewModels/MainViewModel.Heartbeat.cs') -Raw
$viewModelIndicators = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/ViewModels/MainViewModel.Indicators.cs') -Raw
...
$presenter = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/ViewModels/StatusPresenter.cs') -Raw
...
$mainViewModelFiles = Get-ChildItem -LiteralPath (Join-Path $repoRoot 'src/Harness/ViewModels') -File -Filter 'MainViewModel*.cs'
...
$appGlobalAllowedFiles = @(
    'src/Harness/App.xaml.cs',
    'src/Harness/MainWindow.xaml.cs',
    'src/Harness/MainWindow.AlwaysOnTop.cs',
    'src/Harness/MainWindow.Commands.cs',
    'src/Harness/MainWindow.CompactMode.cs',
    'src/Harness/MainWindow.Hotkey.cs',
    'src/Harness/MainWindow.Lifecycle.cs',
    'src/Harness/MainWindow.Theme.cs',
    'src/Harness/MainWindow.Tray.cs',
    'src/Harness/MainWindow.WebView.cs',
    'src/Harness/Views/LogViewerDialog.xaml.cs',
    'src/Harness/Views/SettingsDialog.Actions.cs',
    'src/Harness/Views/SettingsDialog.Initialization.cs',
    'src/Harness/Views/SettingsDialog.Theme.cs',
    'src/Harness/Views/SettingsDialog.xaml.cs'
)
$appGlobalAccessMatches = Get-ChildItem -LiteralPath (Join-Path $repoRoot 'src/Harness') -Recurse -File -Filter *.cs |
...
$fields = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/ViewModels/MainViewModel.Fields.cs') -Raw
...
$coreProperties = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/ViewModels/MainViewModel.Core.Properties.cs') -Raw
...
$commands = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/ViewModels/MainViewModel.Commands.cs') -Raw
...
$mainWindowTray = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/MainWindow.Tray.cs') -Raw
if ($mainWindowTray -match 'StringResources\.Get\("TrayMenu(Open|CompactMode|Exit)"\)' -or
    $mainWindowTray -match '"Open Harness"|"Compact Mode"|"Exit"') {
    throw 'Tray menu labels must use typed StringResources properties instead of raw resource lookup or fallback literals.'
}

$hostedBridge = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/HostedUiBridge.cs') -Raw
if ($hostedBridge -match 'App\.Logger') {
    throw 'HostedUiBridge must use injected IAppLogger, not App.Logger.'
}

$diagnosticService = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/DiagnosticService.cs') -Raw
...
$diagnosticWebViewSession = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/DiagnosticWebViewSession.cs') -Raw
...
$bridgeScript = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/HostedUiBridge.Script.js') -Raw
...
$activityStateScript = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/HostedUiBridge.ActivityState.js') -Raw
$statusInspectionScript = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/HostedUiBridge.StatusInspection.js') -Raw
...
$liveShellSettings = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness.Core/Models/LiveShellSettings.cs') -Raw
$liveShellSettingsChange = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness.Core/Models/LiveShellSettingsChange.cs') -Raw
$liveShellSettingsApplier = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Services/LiveShellSettingsApplier.cs') -Raw
$mainWindowXamlCs = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/MainWindow.xaml.cs') -Raw
if ($liveShellSettings -notmatch 'bool AllowMultipleInstances' -or
    $liveShellSettings -notmatch 'bool AutoRefresh' -or
    $liveShellSettingsChange -notmatch 'DidChangeAllowMultipleInstances' -or
    $liveShellSettingsChange -notmatch 'DidChangeAutoRefresh' -or
    $liveShellSettingsApplier -notmatch 'Action<bool> _applySingleInstancePreference' -or
    $liveShellSettingsApplier -notmatch 'Action<bool> _setAutoRefresh' -or
    $liveShellSettingsApplier -notmatch 'DidChangeAllowMultipleInstances' -or
    $liveShellSettingsApplier -notmatch 'DidChangeAutoRefresh' -or
    $mainWindowXamlCs -notmatch 'ApplyAutoRefresh' -or
    $mainWindowXamlCs -notmatch 'ApplySingleInstancePreference\(allowMultipleInstances\)') {
    throw 'Live shell settings must apply AutoRefresh and AllowMultipleInstances to the running services.'
}

$settingsXaml = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Views/SettingsDialog.xaml') -Raw
...
$commandHelpers = Get-Content -LiteralPath (Join-Path $repoRoot 'src/Harness/Helpers/SimpleCommand.cs') -Raw
