...
Harness 是远程 agent 网关 Web Control UI 的薄桌面外壳。它面向运行在 VPS 上、并通过 Cloudflare Tunnel、反向代理或其他公共 HTTPS 源暴露的部署。
...
本项目最初叫 **OpenClaw Manager**，是专门为 OpenClaw Control UI 写的 WebView2 + WinUI 3 客户端。但实际上其中几乎没有 OpenClaw 专属的东西：窗口 chrome、托盘、全局热键、按环境隔离的会话、心跳、导航恢复、诊断和日志，都是通用的托管 UI 外壳能力。真正知道对端是什么产品的，只有托管页面 URL 和页内 DOM bridge。
...
| 范围 | 变更 |
|---|---|
| 应用名、可执行文件、窗口标题、托盘、关于 | `OpenClaw` → `Harness`（`Harness.exe`） |
| 解决方案 | `OpenClaw.sln` → [Harness.sln](Harness.sln) |
| 外壳项目 | `src/OpenClaw/OpenClaw.csproj` → [src/Harness/Harness.csproj](src/Harness/Harness.csproj) |
| 共享类库 | `src/OpenClaw.Core/OpenClaw.Core.csproj` → [src/Harness.Core/Harness.Core.csproj](src/Harness.Core/Harness.Core.csproj) |
| 测试项目 | `tests/OpenClaw.Core.Tests` → [tests/Harness.Core.Tests](tests/Harness.Core.Tests) |
| 全部命名空间 | `OpenClaw.*` → `Harness.*` |
| 内嵌 bridge 资源名 | `OpenClaw.Services.*.js` → `Harness.Services.*.js` |
| 宿主↔bridge 消息类型 | `openclaw-session-ready` / `-event-gap` / `-control-ui-status` → `harness-*` |
| 单实例锁 / 命名管道 | `OpenClaw.Manager.SingleInstance` → `Harness.SingleInstance` |
| 数据目录 | `%LOCALAPPDATA%\OpenClaw\` → `%LOCALAPPDATA%\Harness\` |
| 日志 / 诊断文件名 | `openclaw-<日期>.log` → `harness-<日期>.log`，`openclaw-diagnostics-*.zip` → `harness-diagnostics-*.zip` |

> **从 5.2.0 或更早版本升级：** 数据目录已迁移，且**不会**自动搬运旧数据。Harness 会以全新配置启动 —— 需要在设置中重新添加网关环境，并重新登录托管 Control UI。旧的 `%LOCALAPPDATA%\OpenClaw\` 目录原样保留，你可以手动把 `settings.json` 拷过去，或在配置完成后自行删除。

### 刻意保持不变的部分

| 范围 | 原因 |
|---|---|
| Gateway 相关文案、`openclaw devices ...` 提示、`__openclaw__/a2ui/` 探测路径 | 这些描述的是 OpenClaw 服务端，不是本客户端 |
| `openclaw-app` DOM 探测和 `readOpenClaw*` bridge 辅助函数 | 它们按名字读取 OpenClaw Control UI 的元素和应用状态 |
| `OPENCLAW_NODE` / `OPENCLAW_ALLOW_NODE_SKIP` 构建脚本变量 | 本地开发环境已经依赖这两个变量 |

---

## 概览

本项目保留现有的托管 Web 体验，同时把它包进一个原生 WinUI 3 窗口，并提供：
...
- `5.3.0` 把应用从 `OpenClaw Manager` 改名为 `Harness`，覆盖解决方案、两个项目、测试项目、全部命名空间、可执行文件以及所有用户可见界面。哪些改了、哪些没改见[为什么改名](#为什么改名)。
- **破坏性变更：** 本地数据目录从 `%LOCALAPPDATA%\OpenClaw\` 迁到 `%LOCALAPPDATA%\Harness\`，且不自动搬运，因此 5.3.0 会以全新配置启动。请在设置中重新添加网关环境，并重新登录托管 Control UI。旧目录原样保留。
- 全新的 Harness 鲸鱼 logo。源文件为 `Assets/harness-logo.png`，窗口/托盘图标、磁贴、启动图、商店图、锁屏图和应用内品牌图都由它生成，`WindowIcon.ico` 现在是多分辨率（16–256px）。
- 发布产物现在会带上应用自己的 `.pri` 和 `Assets/` 目录。在此之前 `dotnet publish` 生成的目录会在创建首个窗口时抛 `XamlParseException`，而且完全没有图标。
...
- 恢复了 Core 回归测试项目（`tests/Harness.Core.Tests`，32 个测试）和 Windows CI workflow；仓库守护脚本从"禁止 tests/"反转为"要求测试项目存在"。
...
- solution 会把当前 `x64`/`x86`/`ARM64` solution platform 下的 `Harness.Core` 映射到 Core project 自身的平台无关 `AnyCPU` 配置，避免 VS2026 打开 Debug/Release x64 时要求手动修 Configuration Manager。
...
| 功能 | 说明 |
|---|---|
| WebView2 Shell | 在原生窗口内托管远程 Control UI |
| Environment Switching | 管理多个托管 Control UI 端点 |
| Connection Status | 状态栏、错误 InfoBar 和重试支持 |
| Auto-Reconnect | 开启 Auto refresh 时，导航失败后自动重试 |
| Heartbeat | 周期性 Control UI 和 transport 探测，支持可配置重连阈值 |
| System Tray | 可配置的关闭到托盘行为；最小化会留在任务栏，并提供打开、重新加载、查看日志、紧凑模式、设置和退出操作（支持中文菜单） |
...
- `Harness.Core/`：纯 .NET 共享代码的物理源码树，供 WinUI app 使用
...
- 提交涉及代码风格的改动前运行 `$env:Platform='x64'; dotnet format Harness.sln --verify-no-changes --no-restore`。
...
1. 用 Visual Studio 2026 打开 [Harness.sln](Harness.sln)。
2. 将解决方案平台设为 `x64`。
3. 按 `F5` 运行。
...
- 在 Harness 中使用公共 HTTPS Control UI URL
...
所有本地数据都存储在 `%LOCALAPPDATA%\Harness\` 下。
